...
class ESTDAN_light_Stack(nn.Module):
    ...
    def forward(self, x):
        ...
        recons_1,flow_forward_recons_1,flow_backward_recons_1, _ = self.recons_net(concated)

        
        concated = torch.stack([frame_list[1], frame_list[2], frame_list[3]], dim=1)
        recons_2, flow_forward_recons_2,flow_backward_recons_2, recons_ortho = self.recons_net(concated)

        
        concated = torch.stack([frame_list[2], frame_list[3], frame_list[4]], dim=1)
        recons_3, flow_forward_recons_3,flow_backward_recons_3, _ = self.recons_net(concated)

        
        concated = torch.stack([recons_1, recons_2, recons_3], dim=1) 

        out, flow_forward_out,flow_backward_out, out_ortho = self.recons_net(concated)
        ...
        return {'recons_1':recons_1, 'recons_2':recons_2, 'recons_3':recons_3, 
                ...
                'recons_ortho':recons_ortho, 'out_ortho':out_ortho}
